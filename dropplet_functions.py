...
def calculate_contact_angle(image_path):
    ...
    if image is None:
        print(f"Error: Image not found or invalid format. Path: {image_path}")
        return
    else:
    ...
    if droplet_contour is not None:
        ...
        # Lets find all the points close to our x, whithing search_range
        ...
        if len(surface_points) < 2:
            ...
            return
        # surface_points = sorted(surface_points, key=lambda p: p[0])  # Sort by x-coordinate
        # cv2.line(result, tuple(surface_points[0]), tuple(surface_points[-1]), (255, 0, 0), 2)  # Blue surface line
        ...
        # cv2.line(result, tuple(filtered_angle_points[0]), tuple(filtered_angle_points[-1]), (0, 0, 255), 2)  # Red surface line
        ...
        plt.axis('off')
        plt.text(0, -40, "Press q to continue to the next image", color='white', fontsize=12, backgroundcolor='black')
        plt.show()
        return angle_deg
    else:
        print("Droplet not detected. Please review the intermediate results.")
